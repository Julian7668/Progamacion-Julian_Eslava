    ...
    (
        answer := questi.text(
            ...
            validate=lambda x: (
                ...
                and (
                    int(match.group(2)) <= int(float(match.group(1))) * 2
                    if match.group(3).capitalize() == "True"
                    else int(match.group(2)) <= int(float(match.group(1)))
                )
            ),
        ).split("/")
    )[0]
...
permite_medios: bool = answer[2].capitalize() == "True"
...
valores_redistribuidos: list[float] = [
